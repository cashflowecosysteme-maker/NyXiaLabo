...
function gameProductPublic(product) {
  ...
  return {
    ...
    gmChoices: product.partyRules?.gmChoices || 'both',
    defaultGmMode: product.partyRules?.gmMode || 'human',
...
function gamePublicRoster(session) {
  const reserved = new Set([...(session.players || []), ...(session.departedPlayers || [])].map(p => p.character?.id).filter(Boolean))
...
function gamePartyRulesFromData(d = {}) {
  const raw = d.partyRules && typeof d.partyRules === 'object' ? d.partyRules : {}
  return {
    characterChoice: ['both', 'create', 'roster'].includes(raw.characterChoice) ? raw.characterChoice : 'both',
    allowSolo: raw.allowSolo !== false,
    allowAiPlayers: raw.allowAiPlayers !== false,
    allowLateJoin: raw.allowLateJoin !== false,
    gmMode: raw.gmMode === 'ai' ? 'ai' : 'human',
    gmChoices: ['both', 'human', 'ai'].includes(raw.gmChoices) ? raw.gmChoices : 'both',
    maxPlayers: Math.max(1, Math.min(GAME_LIVE_MAX_PLAYERS, Number(d.playerCount) || GAME_LIVE_MAX_PLAYERS))
  }
}
...
function gameRuntimeSnapshotFromData(d = {}) {
  return {
    ...
    npcModel: d.npcModel || '',
    aiPlayerPolicy: d.aiPlayerPolicy || '',
    arrivalGuidelines: d.arrivalGuidelines || '',
...
function gameHostPackageFromData(d = {}) {
  return {
    ...
    partyRules: gamePartyRulesFromData(d),
    playerMaterials: d.playerMaterials || '',
    mechanics: d.mechanics || '',
    combatRules: d.combatRules || '',
    progression: d.progression || ''
...
function gameSessionPartyRules(product = {}, overrides = {}) {
  const raw = product.partyRules && typeof product.partyRules === 'object' ? product.partyRules : {}
  const rules = { ...gamePartyRulesFromData({partyRules:raw, playerCount:raw.maxPlayers || Number(product.players) || GAME_LIVE_MAX_PLAYERS}), ...raw }
  const available = rules.gmChoices || 'both'
  const requested = overrides.gmMode === 'ai' ? 'ai' : overrides.gmMode === 'human' ? 'human' : rules.gmMode
  rules.gmMode = available === 'both' || requested === available ? requested : available
  rules.maxPlayers = Math.max(1, Math.min(GAME_LIVE_MAX_PLAYERS, Number(raw.maxPlayers) || Number(product.players) || GAME_LIVE_MAX_PLAYERS))
  return rules
}
async function gameCreateSessionFromProduct(env, product, owner = {}, overrides = {}) {
  ...
  const session = {
    ...
    departedPlayers: [],
    aiBrains: {},
    partyRules: gameSessionPartyRules(product, overrides),
    log: [],
    npcState: {},
...
async function handleGameLibrary(request, env) {
  ...
  if (request.method === 'POST' && path === '/sessions') {
    ...
    const session = await gameCreateSessionFromProduct(env, product, { licenseId: authInfo.license.id }, { teams: body.teams, gmMode: body.gmMode })
...
function gameLivePublicPlayer(p, self = false) {
  ...
  return {
    ...
    team: p.team,
    joinedAt: p.joinedAt,
    controller: p.controller === 'ai' ? 'ai' : 'human',
    lateJoin: p.lateJoin === true,
...
function gameLivePublicState(session, player) {
  ...
  return {
    ...
    partyRules: session.partyRules || {},
    publicEvents: gamePublicEventLog(session),
    runtime: {
...
async function gameLiveNpcReply(env, session, player, npcName, message) {
  ...
  if (!config) throw new Error('Ce PNJ ne dispose pas encore de sa fiche IA privée. Finalise sa préparation dans l’Atelier.')
  ...
  const context = {
    npc: config,
    // Pas de Bible entière ni de secrets d'autres rôles dans le prompt envoyé à OpenRouter.
    memoryRules: config.memoryRules || '',
    relationshipRules: config.relationshipRules || '',
    autonomyRules: config.autonomyRules || '',
...
async function gameOpenRouterBrain(env, session, role, context, instructions, title) {
  if (!env.OPENROUTER_API_KEY) throw new Error('OPENROUTER_API_KEY absente : les cerveaux NyXia Game utilisent OpenRouter.')
  const model = cleanText(session.projectSnapshot?.npcModel || env.NYXIA_GAME_NPC_MODEL || GAME_LIVE_DEFAULT_MODEL, 160)
  const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    method: 'POST',
    headers: { Authorization: `Bearer ${env.OPENROUTER_API_KEY}`, 'Content-Type':'application/json',
      'HTTP-Referer':'https://labo.nyxia.top', 'X-Title':'NyXia Game – cerveau '+title },
    body: JSON.stringify({ model, temperature:0.65, max_tokens:480,
      messages:[{role:'system',content:`Tu es ${role} dans une partie NyXia Game. Tu dois suivre la scène publiée, les connaissances de ton rôle et les règles. Ne révèle jamais un secret non public, n'invente pas de fait canonique et ne modifie jamais l'état du jeu. ${instructions}. Réponds seulement avec ton intervention en français, sans JSON ni commentaires techniques.`},
      {role:'user',content:JSON.stringify(context)}] })
  })
  const data = await response.json().catch(() => ({}))
  if (!response.ok) throw new Error(data?.error?.message || `OpenRouter : HTTP ${response.status}`)
  const answer = cleanText(data?.choices?.[0]?.message?.content || '', 3000)
  if (!answer) throw new Error('OpenRouter n’a fourni aucune intervention.')
  return answer
}
function gamePublicEventLog(session) {
  return (session.log || []).filter(x => ['decision','roll','ai-turn','join','leave','character-created','character-selected'].includes(x.type))
    .slice(-12).map(x => ({type:x.type,playerName:x.playerName,text:x.text||'',result:x.result??null}))
}
function gameArchivePlayer(session, player, reason) {
  if (!player) return
  session.players = (session.players || []).filter(p=>p.id!==player.id)
  session.departedPlayers = Array.isArray(session.departedPlayers) ? session.departedPlayers : []
  session.departedPlayers.push({...player, token:undefined, departedAt:gameLiveNow(), departureReason:reason})
  session.departedPlayers = session.departedPlayers.slice(-100)
  if (session.aiBrains) delete session.aiBrains[player.id]
  gameLiveLog(session,{type:'leave',playerId:player.id,playerName:player.name,text:'Place libérée, progression de la partie conservée.'})
}
function gameAddAiPlayer(session, body={}) {
  const rules = session.partyRules || {}
  if (rules.allowAiPlayers === false) throw new Error('Compagnons joueurs IA non autorisés pour ce jeu.')
  const max = Math.max(1,Number(rules.maxPlayers)||GAME_LIVE_MAX_PLAYERS)
  if ((session.players || []).length >= max) throw new Error('Toutes les places sont déjà occupées.')
  const used = new Set([...(session.players||[]),...(session.departedPlayers||[])].map(p=>p.character?.id).filter(Boolean))
  const requested = cleanText(body.characterId || '',70)
  const selected = requested ? (session.playableCharacters||[]).find(c=>c.id===requested&&!used.has(c.id)) : null
  if (requested && !selected) throw new Error('Personnage IA indisponible ou déjà attribué.')
  if (rules.characterChoice==='roster' && !selected) throw new Error('Ce jeu exige une fiche préétablie : sélectionne un personnage disponible pour le compagnon IA.')
  const aiId = crypto.randomUUID()
  const aiName = cleanText(body.name || selected?.name || `Compagnon IA ${(session.players||[]).filter(p=>p.controller==='ai').length+1}`,80)
  const {aiInstructions,knowledgeIds,...safeCharacter} = selected || {}
  const char = selected ? {...safeCharacter} : {id:crypto.randomUUID(),name:aiName,archetype:'Compagnon',faction:'',publicBio:'',secret:''}
  const teams=session.teams||[]
  const team=cleanText(body.team || teams[(session.players||[]).length%Math.max(1,teams.length)]?.name||'',80)
  const bot={id:aiId,token:null,controller:'ai',name:aiName,team,character:char,
    badges:[],reputation:{},joinedAt:gameLiveNow(),lastSeenAt:gameLiveNow()}
  session.players=[...(session.players||[]),bot]
  session.aiBrains=session.aiBrains||{}
  session.aiBrains[bot.id]={instructions:cleanText(aiInstructions||session.projectSnapshot?.aiPlayerPolicy||'',6000),knowledgeIds:Array.isArray(knowledgeIds)?knowledgeIds:[],lastAt:0}
  gameLiveLog(session,{type:'ai-added',playerId:bot.id,playerName:bot.name,text:'Compagnon IA ajouté.'})
  return bot
}
async function gameAiPlayerTurn(env,session,aiId) {
  const bot=(session.players||[]).find(p=>p.id===aiId&&p.controller==='ai')
  if (!bot) throw new Error('Compagnon IA introuvable.')
  session.aiBrains=session.aiBrains||{}
  const brain=session.aiBrains[aiId]||{}
  if (Date.now()-(brain.lastAt||0)<2500) throw new Error('Patiente un instant avant le prochain tour IA.')
  const runtime=session.runtime||{}
  const answer=await gameOpenRouterBrain(env,session,`le personnage joueur IA ${bot.character?.name||bot.name}`,
    {personnage:{name:bot.character?.name,archetype:bot.character?.archetype,faction:bot.character?.faction,motivation:bot.character?.motivation},
     scène:{titre:runtime.sceneTitle,objectif:runtime.objective,annonce:runtime.announcement,narration:runtime.narrative},
     indicesPublics:runtime.sharedClues||[],journalPublic:gamePublicEventLog(session)},
    brain.instructions||'Tu es un compagnon de jeu. Tu prends une initiative compatible avec la scène, sans parler pour les humains.', 'PJ IA')
  brain.lastAt=Date.now();session.aiBrains[aiId]=brain
  gameLiveLog(session,{type:'ai-turn',playerId:bot.id,playerName:bot.name,team:bot.team,text:answer})
  return {playerId:bot.id,name:bot.name,reply:answer}
}
async function gameAiGmAct(env,session,body) {
  if(session.partyRules?.gmMode!=='ai') throw new Error('Ce jeu utilise actuellement un maître de jeu humain.')
  if(session.status==='ended') throw new Error('Partie terminée.')
  if(session.status==='lobby'&&session.partyRules?.allowSolo===false&&(session.players||[]).filter(p=>p.controller!=='ai').length<2) throw new Error('Ce jeu exige au moins deux joueurs humains pour commencer.')
  const scenes=session.guidedScenes||[]
  if(!scenes.length) throw new Error('Aucune scène guidée : termine le conducteur dans l’Atelier.')
  if (Date.now() - (Number(session.aiGmLastAt)||0) < 3500) throw new Error('Patiente quelques secondes avant une autre intervention du MJ IA.')
  const current=Math.max(0,Math.min(scenes.length-1,Number(session.runtime?.guidedSceneIndex)||0))
  if(Number(body.expectedIndex)!==current) throw new Error('La scène a changé. Actualise avant de continuer.')
  const action=body.action==='next'?'next':'start'
  const next=action==='next'?current+1:current
  if(next>=scenes.length) throw new Error('Dernière scène atteinte. Le MJ peut terminer la partie.')
  const scene=scenes[next]
  const narration=await gameOpenRouterBrain(env,session,'le maître de jeu IA',
    {scèneApprouvée:{titre:scene.title,phase:scene.phase,objectifPublic:scene.playerObjective,annonce:scene.announcement,texteALire:scene.readAloud},
     journalPublic:gamePublicEventLog(session),personnagesActifs:(session.players||[]).map(p=>({name:p.character?.name||p.name,controller:p.controller||'human'}))},
    'Anime la scène actuelle avec 2 à 4 phrases. Ne décris pas une scène ultérieure. Aucun indice caché ou solution ne doit être révélé.', 'MJ IA')
  const previous=session.runtime||{}
  const updated=gameRuntimeFromGuidedScene(scene,next)
  updated.sharedClues=previous.sharedClues||[]
  updated.teamClues=previous.teamClues||{}
  updated.narrative=narration
  session.runtime=gameLiveCleanRuntime(updated)
  session.status='live'
  session.aiGmLastAt=Date.now()
  gameLiveLog(session,{type:'ai-gm',text:`Scène ${next+1} : ${scene.title}`,sceneId:scene.id})
  return {reply:narration,index:next}
}
async function handleGamePublic(request, env) {
  ...
  if (request.method === 'POST' && path === '/join') {
    ...
    if (!player && priorToken) return json({ error:'Ton ancien accès a été fermé. Rejoins comme nouvelle personne sans reprendre le personnage précédent.' },401)
    if (!player) {
      const rules=session.partyRules||{}
      if (session.status!=='lobby' && rules.allowLateJoin===false) return json({error:'Ce jeu n’accepte pas de nouveaux arrivants après le début.'},409)
      const max=Math.max(1,Number(rules.maxPlayers)||GAME_LIVE_MAX_PLAYERS)
      if ((session.players||[]).length>=max) {
        const replaceable=(session.players||[]).find(p=>p.controller==='ai')
        if (!replaceable) return json({error:'Partie complète : aucune place IA disponible à libérer.'},409)
        gameArchivePlayer(session,replaceable,'Remplacé par une nouvelle personne — personnages distincts')
      }
      const teams=session.teams||[]
      const team=cleanText(body.team||teams[(session.players||[]).length%Math.max(1,teams.length)]?.name||'',80)
      player={id:crypto.randomUUID(),token:crypto.randomUUID().replaceAll('-',''),controller:'human',
        name,team,character:null,lateJoin:session.status!=='lobby',badges:[],reputation:{},joinedAt:gameLiveNow(),lastSeenAt:gameLiveNow()}
      ...
      gameLiveLog(session,{type:'join',playerId:player.id,playerName:player.name,team:player.team,
        text:'Nouveau joueur : nouveau personnage indépendant, progression collective conservée.'})
    } else {player.name=name;player.lastSeenAt=gameLiveNow()}
    await gameLiveSave(env,session)
  ...
  if (request.method === 'POST' && path === '/leave') {
    gameArchivePlayer(session,player,'Départ volontaire')
    await gameLiveSave(env,session)
    return json({ok:true,continued:true})
  }
  if (request.method === 'POST' && path === '/ai-turn') {
    if (session.partyRules?.allowAiPlayers===false) return json({error:'Compagnons IA non autorisés.'},403)
    try {const result=await gameAiPlayerTurn(env,session,cleanText(body.aiId||'',100));await gameLiveSave(env,session);return json({ok:true,...result})}
    catch(e){return json({error:e.message||'Échec OpenRouter'},409)}
  }
  if (request.method === 'POST' && path === '/ai-gm') {
    if(!player.character) return json({error:'Crée ton personnage avant de lancer le MJ IA.'},409)
    try {const result=await gameAiGmAct(env,session,body);await gameLiveSave(env,session);return json({ok:true,...result,state:gameLivePublicState(session,player)})}
    catch(e){return json({error:e.message||'Échec OpenRouter MJ'},409)}
  }
  ...
  if (request.method === 'POST' && path === '/character') {
    ...
    const mode=session.partyRules?.characterChoice||'both'
    if (mode !== 'create' && body.characterId) {
    ...
    if(mode==='roster' && player.lateJoin!==true) return json({error:'Choisis un personnage prédéfini pour ce jeu.'},400)
    if(player.character?.id) return json({error:'Ce personnage existe déjà dans cette partie. Utilise un autre joueur pour en créer un nouveau.'},409)
  ...
  if (request.method === 'POST' && path === '/npc-chat') {
    ...
    let answer
    try {answer = await gameLiveNpcReply(env, session, player, npcName, message)}
    catch(e){return json({error:e.message||'Erreur du PNJ OpenRouter'},409)}
...
async function gameLiveApplyHostUpdate(session, body = {}) {
  if (body.gmMode) {
    const mode=body.gmMode
    const available=session.partyRules?.gmChoices||'both'
    if(!['human','ai'].includes(mode)|| (available!=='both'&&mode!==available)) throw new Error('Ce mode MJ n’est pas autorisé pour ce jeu.')
    if(mode==='ai'&&!(session.guidedScenes||[]).length) throw new Error('Prépare les scènes guidées avant d’activer le MJ IA.')
    session.partyRules=session.partyRules||{};session.partyRules.gmMode=mode
    gameLiveLog(session,{type:'gm-change',text:mode==='ai'?'MJ IA activé (OpenRouter)':'MJ humain activé'})
  }
  if(body.participant&&typeof body.participant==='object'){
    const op=body.participant
    if(op.action==='add-ai') gameAddAiPlayer(session,op)
    else if(op.action==='remove'){
      const bot=(session.players||[]).find(p=>p.id===op.playerId&&p.controller==='ai')
      if(!bot) throw new Error('Sélectionne un compagnon IA actif.')
      gameArchivePlayer(session,bot,'Place IA libérée par le MJ')
    } else if(op.action==='remove-human'){
      const human=(session.players||[]).find(p=>p.id===op.playerId&&p.controller!=='ai')
      if(!human) throw new Error('Joueur humain introuvable.')
      gameArchivePlayer(session,human,'Retiré par le MJ : ancien jeton révoqué')
    } else throw new Error('Commande participant inconnue.')
  }

  if (body.status) {
    if(body.status==='live'&&session.status==='lobby'&&session.partyRules?.allowSolo===false&&(session.players||[]).filter(p=>p.controller!=='ai').length<2) throw new Error('Ce jeu exige au moins deux joueurs humains pour commencer.')
...
async function handleGameCustomerHost(request, env) {
  ...
  if (request.method === 'PUT') {
    try {await gameLiveApplyHostUpdate(session, body)} catch(e){return json({error:e.message},409)}
    await gameLiveSave(env, session)
...
async function handleGameHost(request, env) {
  ...
  if (request.method === 'POST' && !code) {
    ...
    const session = await gameCreateSessionFromProduct(env, testProduct, { licenseId: 'ATELIER-TEST' }, { teams: body.teams || d.liveTeams, gmMode: body.gmMode })
