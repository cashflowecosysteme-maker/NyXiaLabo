...
function normalizeScene(scene,i){
 ...
 return {id,act,title:titleOf(scene,i),announcement:String(scene?.announcement||'').slice(0,3500),notes:String(scene?.notes||'').slice(0,5000),slots};
}
...
function fromGuide(guide){return (Array.isArray(guide)?guide:guide.scenes||[]).map((s,i)=>{
 ...
 return normalizeScene({id:s.id,act:s.phase,title:s.title,announcement:s.announcement,slots},i)
});}
function state(){
...
function syncSlides(t){
 ...
 const scenes=t.scenes.map((s,i)=>{
  ...
  const media={items,slides:items.map((x,n)=>({slideId:s.id+'-'+x.id,index:n,...x})),imageUrl:items.find(x=>x.kind==='image')?.url||'',audioUrl:items.find(x=>x.kind==='audio')?.url||'',videoUrl:items.find(x=>x.kind==='video')?.url||'',instruction:old.media?.instruction||''};
...
window.nyxSceneSet=function(i,field,value){const t=state(),s=t.scenes[i];if(!s||!['act','title','announcement'].includes(field))return;s[field]=String(value||'').slice(0,3500);commit(t,false)};
...
window.gameMedia=function(d){const t=state(),ready=t.scenes.length,slot=pickSlot(),n=t.scenes.reduce((n,s)=>n+s.slots.filter(x=>x.required&&!safeUrl(x.url)).length,0),kind=slot?.kind||'image';
 ...
 '<div class="divider"></div><h3>🎞️ Montage par acte et scène</h3>'+(ready?t.scenes.map((s,i)=>'<details '+(i===selection.scene?'open':'')+' class="panel" style="margin:12px 0"><summary style="cursor:pointer;font-size:16px;font-weight:700">'+e(s.act+' — '+s.title)+' · '+s.slots.filter(m=>safeUrl(m.url)).length+'/'+s.slots.length+' URL</summary><div class="row" style="margin-top:12px"><div class="field"><label>Acte</label><input value="'+e(s.act)+'" onchange="nyxSceneSet('+i+',\'act\',this.value)"></div><div class="field"><label>Scène</label><input value="'+e(s.title)+'" onchange="nyxSceneSet('+i+',\'title\',this.value)"></div></div><div class="field"><label>📢 Annonce (affichée sur une diapositive distincte)</label><textarea rows="2" onchange="nyxSceneSet('+i+',\'announcement\',this.value)">'+e(s.announcement)+'</textarea></div><div class="tools-line"><button class="btn" onclick="nyxSceneMove('+i+',-1)"'+(i===0?' disabled':'')+'>↑ Scène</button><button class="btn" onclick="nyxSceneMove('+i+',1)"'+(i===t.scenes.length-1?' disabled':'')+'>↓ Scène</button>'+Object.keys(KINDS).map(k=>'<button class="btn" onclick="nyxSlotAdd('+i+',\''+k+'\')">+ '+e(kindOf(k))+'</button>').join('')+'</div>'+s.slots.map((m,j)=>slotHtml(s,i,m,j,t)).join('')+'</details>').join(''):'<div class="game-note">Importe le Cahier Média : les actes et scènes apparaîtront ici, sans bibliothèque de médias à parcourir.</div>')+
 '<div class="panel"><h4>▶ Présentation « Le jeu »</h4><p class="tiny">'+t.scenes.length+' scènes ; '+t.scenes.reduce((v,s)=>v+1+s.slots.filter(m=>safeUrl(m.url)).length,0)+' diapositives (annonce + médias). Les URL sont synchronisées automatiquement au conducteur. Boutons Suivant, Précédent et Reprendre dans le portail.</p><button class="btn primary" onclick="nyxMediaCommit()"'+(!ready?' disabled':'')+'>✓ Synchroniser les diapositives</button><button class="btn gold" onclick="setGameTab(\'package\')">📦 Compilation ZIP →</button></div>';
};
